...
def get_player(data: dict, uid: int, username: str = "") -> dict:
    ...
    if key not in data:
        data[key] = {
            ...
            "items": {}, "joined": msk_now().strftime("%Y-%m-%d %H:%M:%S"),
        }
        # статистика по дням
        stats = load_stats()
...
RANKS = [
    ...
    {"name": "Генерал-майор",     "min_xp": 11000,  "emoji": "🎖️", "idx": 10},
...
async def cmd_help(msg: Message):
    await msg.answer(
        ...
        "ВСЕМ:\n"
        "/ticket — Штраф, КД 30 мин\n"
        "/bribe — Взятка, КД 30 мин\n"
    ...
    await msg.answer(
        ...
        "/buy предмет — купить\n\n"
        "РП КОМАНДЫ в чате:\n"
        "Ответьте на сообщение без слеша:\n"
        "надеть_наручники, обыск, тайзер,\n"
        "розыск, арест, допрос, слежка и др.\n"
        "Полный список: /rp_help"
    )

# ══════════════════════════════════════════
#  /rp_help
# ══════════════════════════════════════════
@dp.message(Command("rp_help", "rphelp"))
...
#  РП ОБРАБОТЧИК
# ══════════════════════════════════════════
@dp.message(F.text.func(lambda t: t is not None and t.strip().lower() in RP_COMMANDS))
...
async def cmd_pass(msg: Message):
    ...
    if nxt:
        ...
        prog = "⬆️ До " + nxt["emoji"] + " " + nxt["name"] + ": " + str(nxt["min_xp"]-xp) + " XP\n`" + bar + "`"
    else:
    ...
    await msg.answer(
        ...
        + "━━━━━━━━━━━━━━━━━━\n"
        + "⚡ Опыт: " + str(xp) + " XP\n"
        + "💰 Монеты: " + str(coins) + radar_info + "\n\n"
        + prog + "\n\n"
        + "━━━━━━━━━━━━━━━━━━\n"
        + "СТАТИСТИКА\n"
        + "Погонь: " + str(p.get("pursuits",0)) + " (успех: " + str(p.get("pursuits_success",0)) + ")\n"
        + "Задержаний: " + str(p.get("cuffs",0)) + "\n"
        + "Операций: " + str(p.get("operations",0)) + "\n"
...
async def cmd_top(msg: Message):
    ...
    for i, (uid, p) in enumerate(pl[:10]):
        ...
        text += m + " @" + name + " — " + rank["emoji"] + " " + rank["name"] + " | ⚡ " + str(p.get("xp",0)) + " | 🎯 " + str(p.get("catches",0)) + "\n"
    await msg.answer(text)
...
async def cmd_rank(msg: Message):
    ...
    for r in RANKS:
        ...
        text += r["emoji"] + " " + r["name"] + " — " + str(r["min_xp"]) + " XP" + me + ul + "\n"
    await msg.answer(text)
...
def admin_menu_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="👥 Все пользователи",  callback_data="admin_users_xp_desc")],
    ])

def users_filter_kb(current: str) -> InlineKeyboardMarkup:
    filters = [
        ("🆕 Сначала новые",    "admin_users_new"),
        ("🕰 Сначала старые",   "admin_users_old"),
        ("⬆️ Больше опыта",    "admin_users_xp_desc"),
        ("⬇️ Меньше опыта",   "admin_users_xp_asc"),
        ("💰 Больше монет",    "admin_users_coins_desc"),
        ("💸 Меньше монет",    "admin_users_coins_asc"),
    ]
    rows = []
    for label, cb in filters:
        mark = " ✅" if cb == current else ""
        rows.append([InlineKeyboardButton(text=label+mark, callback_data=cb)])
    rows.append([InlineKeyboardButton(text="◁ Назад", callback_data="admin_panel")])
    return InlineKeyboardMarkup(inline_keyboard=rows)

def build_users_page(sort_key: str) -> str:
    data = load_data()
    players = list(data.items())

    if sort_key == "admin_users_new":
        players.sort(key=lambda x: x[1].get("joined", "0000-00-00"), reverse=True)
    elif sort_key == "admin_users_old":
        players.sort(key=lambda x: x[1].get("joined", "9999-99-99"))
    elif sort_key == "admin_users_xp_desc":
        players.sort(key=lambda x: x[1].get("xp", 0), reverse=True)
    elif sort_key == "admin_users_xp_asc":
        players.sort(key=lambda x: x[1].get("xp", 0))
    elif sort_key == "admin_users_coins_desc":
        players.sort(key=lambda x: x[1].get("coins", 0), reverse=True)
    elif sort_key == "admin_users_coins_asc":
...
# ── Все пользователи ────────────────────
USERS_SORT_KEYS = {
    "admin_users_new", "admin_users_old",
    "admin_users_xp_desc", "admin_users_xp_asc",
    "admin_users_coins_desc", "admin_users_coins_asc",
}

@dp.callback_query(F.data.in_(USERS_SORT_KEYS))
async def cb_admin_users(cb: CallbackQuery):
    if (cb.from_user.username or "").lower() != ADMIN_USERNAME.lower():
        return await cb.answer()
    sort_key = cb.data
    text = build_users_page(sort_key)
    kb = users_filter_kb(sort_key)
    try:
        await cb.message.edit_text(text, reply_markup=kb)
    except Exception:
